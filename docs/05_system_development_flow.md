...
【データセンター側】
  ...
  │  │    │                  │            │            │       │
  │  ▼    ▼                  ▼            ▼            ▼       │
  │ QNAP  QNAP            GPU Server  SeaweedFS  Zabbixサーバー │
  │ NAS1  NAS2            A6000×12    HDDサーバー               │
  │ NAS3  NAS4            │           ×4台                     │
  │  │    (M.2 NVMe       │         (250TB/台=1PB合計)        │
  │  │   ホットキャッシュ)   │           │                     │
  │  │    │              │           │ SeaweedFS               │
  │  │    ▼←───────────┘           │ 分散ストレージ          │
  │  │  アクティブジョブデータキャッシュ       │ (Master/Volume/Filer) │
  │  │    │                         │                     │
  │  │    ▼←───────────────────┘                     │
  │  │  ジョブ完了データ保存・S3バックアップ                        │
  └────────────────────────────────────────────────────────────┘
...
> 📌 **各機器の役割**:
> - **QNAP NAS × 4台 (M.2 NVMe)**: アクティブジョブのホットキャッシュ・バッファ（高速読み書き用）
> - **SeaweedFS HDDサーバー × 4台 (250TB/台)**: 分散ストレージ本体（Master/Volume/Filer機能を担当、実データ保存）
...
| QNAP NAS | M.2 NVMe 4TB × 8枚 = 32TB/台 | 4台 | **ホットキャッシュ・アクティブジョブバッファ**（高速読み書き） |
| SeaweedFS HDDサーバー | 250TB/台（機種・構成未定） | 4台 | **SeaweedFS 分散ストレージ**（実データ 1PB合計） |
...
### 2.3 役割夢り当て: QNAP vs SeaweedFS HDDサーバー

| 機器 | 役割 | 理由 |
|------|------|------|
| **QNAP NAS × 4台** (M.2 NVMe) | ホットキャッシュ・アクティブジョブバッファ | NVMeの高速アクセス性を活かし、GPUサーバーとの遠距間高速読み書きに最適 |
| **SeaweedFS HDDサーバー × 4台** (250TB) | SeaweedFS 分散ストレージ本体 | HDDの大容量・低コストを活かし、大量データの分散・冗長保存に最適 |

### 2.4 SeaweedFS クラスター構成（**検証中**）

> 📌 **前提**: SeaweedFSはHDDサーバー 4台上で動作させる。各HDDサーバーがSeaweedFSのロール（Master/Volume/Filer等）を分担する。

| ロール | 実行場所 | 台数 | 役割 |
|------|---------|------|------|
| ロードバランサー (HAProxy) | HDDサーバー or 独立VM | 2台 (冗長) | クライアント→Filerへの振り分け |
| Filerサーバー | HDDサーバー上 (VM/プロセス) | 2台 | クライアントアクセス用 |
| Masterサーバー | HDDサーバー上 (VM/プロセス) | 3台 | メタデータ管理・Raft選出 |
| Databaseサーバー | HDDサーバー上 (VM/プロセス) | 3台 | メタデータ永続化 |
| Volumeサーバー | HDDサーバー上 (物理ディスクダイレクトマウント) | **4台** | 実データ保存 (250TB×4台=1PB) |

> ⚠️ **ステータス**: 現在検証環境で性能試験・フェイルオーバー試験を実施中。合格後にサービス環境を構築する予定。

#### HDDサーバー 4台へのSeaweedFSロール配置例

```
HDDサーバー 1: Volumeサーバー (250TB) + Master1 + Filer1 + LB1 (HAProxy)
HDDサーバー 2: Volumeサーバー (250TB) + Master2 + Filer2 + LB2 (HAProxy)
HDDサーバー 3: Volumeサーバー (250TB) + Master3 + Database1
HDDサーバー 4: Volumeサーバー (250TB) + Database2 + Database3

実効ストレージ: 1PB💾 (250TB×4台)
```
